...
CF_SINGLES="${CF_SINGLES:-12}"
CF_NGRAMS_PER_BUCKET="${CF_NGRAMS_PER_BUCKET:-5}"
CF_SPLICE_EPSILON="${CF_SPLICE_EPSILON:-0.05}"
CF_TELEPORT="${CF_TELEPORT:-0.15}"
CF_START_FLOOR="${CF_START_FLOOR:-0.1}"
CF_WALK_STALL="${CF_WALK_STALL:-20000}"
...
CF_SINGLES=$CF_SINGLES
CF_NGRAMS_PER_BUCKET=$CF_NGRAMS_PER_BUCKET
CF_SPLICE_EPSILON=$CF_SPLICE_EPSILON
CF_TELEPORT=$CF_TELEPORT
CF_START_FLOOR=$CF_START_FLOOR
CF_WALK_STALL=$CF_WALK_STALL
...
run_algorithm() {
  ...
  if [[ "$RUN_CHUNK_FOREST" == "1" ]]; then
    ...
    run_cmd python3 -m llvm_ir.stages.translation_unit.evaluate_chunk_forest \
      ...
      --max-length "$MAX_LENGTH" \
      --singles "$CF_SINGLES" \
      --ngrams-per-bucket "$CF_NGRAMS_PER_BUCKET" \
      --splice-epsilon "$CF_SPLICE_EPSILON" \
      --teleport "$CF_TELEPORT" \
      --start-floor "$CF_START_FLOOR" \
      --walk-stall "$CF_WALK_STALL" \
