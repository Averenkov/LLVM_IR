"""Strong-subsequence beam heuristic on whole translation units.

Leaves are *common subsequences* (order-preserving, non-contiguous pass patterns
shared by >= min_support functions; see mine_common_subsequences) measured on the
TU. They are sorted and fed into the include/skip beam merge from
``segment_tree_beam`` (segment_tree_beam_merge), which at every node beam-searches
both concatenation orders and is guaranteed never worse than the plain
best-prefix concatenation. The root's best sequence is reported.

This is the subsequence-leaf cousin of strong_links (which mines contiguous
substrings and merges with a graph-guided beam).
"""
...
from llvm_ir.heuristics.translation_unit.strong_links import mine_common_subsequences
...
def _row(benchmark, bitcode_path, passes, best_size, baseline_size, oz_size, baseline_instr, oz_instr, extra):
    return {
        ...
        "error": "" if passes else "no subsequences", "error_kind": "" if passes else "full_failure",
...
def evaluate_for_benchmark(benchmark, function_results, bitcode_path, *, args):
    with tempfile.TemporaryDirectory(prefix="llvm-ir-subseq-") as tmp_str:
        ...
        node_beam = int(getattr(args, "node_beam", 8) or 1)
        ...
        # ---- Phase 1: common-subsequence leaves ----
        links = mine_common_subsequences(
            function_results,
        ...
        if getattr(args, "add_singles", True):
            # Fallback for benchmarks with few functions (no subsequence reaches
            # min_support): seed single-pass leaves so the beam still has material.
            have = {sub for sub, _ in links}
            counts: dict[tuple[str, ...], int] = {}
            positives = [r for r in function_results if r.delta > 0] or function_results
            for r in positives:
                for p in r.passes:
                    counts[(p,)] = counts.get((p,), 0) + 1
            links = links + [(s, w) for s, w in counts.items() if s not in have]
        # size-scaled cap on how many leaves we actually measure on the TU
        size_scale = min(args.max_size_scale, max(1.0, len(links) / max(1, args.size_ref)))
        eff_select = max(args.min_budget, round(args.select_count * size_scale))
        selected = links[:eff_select]
        leaves: list[tuple[tuple[str, ...], int]] = []
        for sub, _weight in selected:
            best_size, best_passes = measure(sub)
            if best_passes:
                leaves.append((best_passes, best_size))
        leaf_evals = len(prefix_cache) - 1

        # ---- Phase 2: include/skip beam merge (never worse than best-prefix) ----
        leaves.sort(key=lambda item: (item[1], item[0]))
        if not leaves:
            best_passes, best_size = (), baseline_size
        elif node_beam_eff > 1:
            ...
            (best_passes, best_size), _ = segment_tree_merge(leaves, measure, max_length=args.max_length)
        if not best_passes or best_size is None:
            best_passes, best_size = (), baseline_size
        merge_evals = (len(prefix_cache) - 1) - leaf_evals

        extra = {
            "links_mined": len(links), "leaves": len(leaves), "eff_select": eff_select,
            "node_beam": node_beam_eff, "crashing_passes": sorted(crashing),
...
def _run_one(benchmark, function_results, bitcode_path, args):
    return benchmark, evaluate_for_benchmark(benchmark, function_results, bitcode_path, args=args)
...
def build_report(groups, bitcode_paths, args) -> dict[str, Any]:
    ...
    if jobs == 1:
        for index, benchmark in enumerate(benchmarks, start=1):
            print(f"[{index}/{len(benchmarks)}] {benchmark}: {HEURISTIC}", flush=True)
            results[benchmark] = evaluate_for_benchmark(
...
def parse_args(argv: list[str] | None = None) -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Strong-subsequence beam heuristic on whole TUs.")
    parser.add_argument("--comparison", required=True, type=Path)
    parser.add_argument("--algorithm", default="")
    parser.add_argument("--bitcode-dir", required=True, type=Path)
    parser.add_argument("--output", required=True, type=Path)
    parser.add_argument("--min-support", type=int, default=2, help="Min functions sharing a subsequence.")
    parser.add_argument("--max-links", type=int, default=2000, help="Cap on mined subsequences (0=all).")
    parser.add_argument("--max-subseq-len", type=int, default=4, help="Max subsequence length mined.")
    parser.add_argument("--add-singles", action=argparse.BooleanOptionalAction, default=True,
                        help="Seed single-pass leaves (fallback for few-function benchmarks).")
    parser.add_argument("--select-count", type=int, default=300, help="Leaves measured on the TU.")
    parser.add_argument("--max-length", type=int, default=16, help="Cap on merged-sequence length.")
    parser.add_argument("--size-ref", type=int, default=200)
    parser.add_argument("--max-size-scale", type=float, default=3.0)
    parser.add_argument("--min-budget", type=int, default=16)
    parser.add_argument("--node-beam", type=int, default=8,
                        help="Merge-node beam width (include/skip DP). 1 = plain best-prefix merge.")
    parser.add_argument("--node-beam-min", type=int, default=1)
    parser.add_argument("--node-beam-size-ref", type=int, default=60000,
                        help="Baseline .text (bytes) that gets the full --node-beam; bigger modules get less.")
