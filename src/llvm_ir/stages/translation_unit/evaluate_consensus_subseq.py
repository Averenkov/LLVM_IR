"""Two-wave consensus-subsequence heuristic with a whole-TU teacher.

Wave 1 mines the largest common (gapped) subsequences of the per-function best
sequences, generates noisy realizations (gaps filled from a start-frequency +
catalog exploration distribution), and measures them on the TU. The teacher then
re-weights the exploration distribution by the measured per-pass ``.text`` byte
marginals, and wave 2 regenerates noisy candidates against that calibrated
distribution. The best measured prefix across both waves is reported.
"""
...
HEURISTIC = "consensus_subseq"


def pass_marginals_from_cache(paths, prefix_cache) -> dict[str, float]:
    """Mean measured ``.text`` byte gain per pass (node-level credit) from a TU run."""
    observations: dict[str, list[int]] = {}
    for path in paths:
        prev = prefix_cache.get(())
        for index, pass_name in enumerate(path):
            ...
            if prev is not None and not prev.get("error"):
                observations.setdefault(pass_name, []).append(int(prev["size"]) - int(entry["size"]))
            prev = entry
    return {p: sum(v) / len(v) for p, v in observations.items()}


def calibrate_exploration(base: dict[str, float], measured: dict[str, float]) -> dict[str, float]:
    """Re-weight the filler distribution by measured per-pass TU marginals.

    Passes with a positive measured byte gain take that value; the rest keep their
    start-frequency weight rescaled onto the measured byte scale, so both live on a
    comparable scale. A small floor keeps every pass reachable.
    """
    positive = [v for v in measured.values() if v > 0]
    base_for_positive = [base.get(p, 0.0) for p, v in measured.items() if v > 0]
    denom = sum(base_for_positive) / len(base_for_positive) if base_for_positive else 0.0
    scale = (sum(positive) / len(positive)) / denom if positive and denom > 0 else 1.0
    floor = min(base.values()) if base else 1.0
    out: dict[str, float] = {}
    for p, base_w in base.items():
        m = measured.get(p, None)
        if m is not None and m > 0:
            out[p] = m + floor
        else:
            out[p] = max(base_w * scale, floor)
    return out
...
def _empty_row(benchmark, bitcode_path, baseline_size, oz_size, baseline_instr, oz_instr):
    return {
        ...
        "error": "no consensus candidates", "error_kind": "full_failure",
        "baseline_instruction_count": baseline_instr, "oz_instruction_count": oz_instr,
        "oz_instruction_delta": (baseline_instr - oz_instr) if baseline_instr is not None and oz_instr is not None else None,
...
def _ranked_pool(pool, top_k):
    # Prefer longer (more consensus content), then deterministic order.
    return sorted(pool, key=lambda p: (-len(p), p))[: max(1, top_k)]


def evaluate_consensus_subseq_for_benchmark(benchmark, function_results, bitcode_path, *, args):
    with tempfile.TemporaryDirectory(prefix="llvm-ir-consensus-") as tmp_str:
        ...
        config = ConsensusConfig(
            minsup=args.minsup, max_pattern_length=args.max_pattern_length, beam=args.beam,
            top_skeletons=args.top_skeletons, max_length=args.max_length,
            epsilon_insert=args.epsilon_insert, epsilon_drop=args.epsilon_drop,
            swap_prob=args.swap_prob, pool_size=args.pool_size, walk_seed=args.walk_seed,
        )
        skeletons = mine_frequent_subsequences(function_results, config=config)
        base_explore = exploration_distribution(function_results, graph.start_counts, filler_floor=config.filler_floor)
        budget = args.max_real_evals_per_benchmark
        excluded: set[tuple[str, ...]] = set()

        # ---- Wave 1 ----
        pool1 = generate_candidate_pool(skeletons, base_explore, config=config, rng=Random(config.walk_seed))
        rows1, evaluated1 = _evaluate(
            _ranked_pool(pool1, args.top_k), benchmark=benchmark, bitcode_path=bitcode_path, workdir=workdir,
            prefix_cache=prefix_cache, wave=1, offset=0, budget=budget,
            baseline_size=baseline_size, oz_size=oz_size, baseline_instr=baseline_instr, oz_instr=oz_instr,
            excluded=excluded,
        )
        wave1_evals = len(prefix_cache) - 1

        # ---- Teacher: recalibrate exploration with measured per-pass marginals ----
        measured = pass_marginals_from_cache(evaluated1, prefix_cache)
        rows2: list[dict[str, Any]] = []
        calibrated_passes = sum(1 for v in measured.values() if v > 0)
        if args.waves > 1 and measured and (budget <= 0 or (len(prefix_cache) - 1) < budget):
            calibrated = calibrate_exploration(base_explore, measured)
            pool2 = generate_candidate_pool(skeletons, calibrated, config=config, rng=Random(config.walk_seed + 1))
            rows2, _ = _evaluate(
                _ranked_pool(pool2, args.top_k), benchmark=benchmark, bitcode_path=bitcode_path, workdir=workdir,
        ...
        common = {
            ...
            "longest_skeleton": max((len(s.passes) for s in skeletons), default=0),
            "calibrated_passes": calibrated_passes,
            "crashing_passes": sorted(crashing),
            "wave1_real_evals": wave1_evals, "wave2_real_evals": wave2_evals,
            "real_evals": len(prefix_cache) - 1, "waves": args.waves,
        }
        if candidate_rows:
...
def _run_one(benchmark, function_results, bitcode_path, args):
    return benchmark, evaluate_consensus_subseq_for_benchmark(benchmark, function_results, bitcode_path, args=args)
...
def build_report(groups, bitcode_paths, args) -> dict[str, Any]:
    ...
    if jobs == 1:
        for index, benchmark in enumerate(benchmarks, start=1):
            print(f"[{index}/{len(benchmarks)}] {benchmark}: consensus_subseq waves={args.waves}", flush=True)
            results[benchmark] = evaluate_consensus_subseq_for_benchmark(
...
def parse_args(argv: list[str] | None = None) -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Two-wave consensus-subsequence heuristic with whole-TU teacher.")
    ...
    parser.add_argument("--top-k", type=int, default=250, help="Candidates evaluated per wave.")
    parser.add_argument("--minsup", type=int, default=2)
    parser.add_argument("--max-pattern-length", type=int, default=8)
    parser.add_argument("--beam", type=int, default=200)
    parser.add_argument("--top-skeletons", type=int, default=60)
    parser.add_argument("--max-length", type=int, default=12)
    parser.add_argument("--epsilon-insert", type=float, default=0.25)
    parser.add_argument("--epsilon-drop", type=float, default=0.1)
