"""Graph-informed beam segment-tree heuristic on whole translation units.

Identical pipeline to ``segment_tree_beam`` (measured 4-pass leaves -> balanced
binary tree -> include/skip beam merge), but the beam's per-node pruning is
**graph-informed**: besides the top ``node_beam`` paths by measured ``.text``,
up to ``node_graph_slots`` extra paths are kept even when they are currently
*worse*, provided their last pass has a strong pass-order-graph edge to the next
pass in the order (and they are within ``node_graph_tol`` of the best size).

Rationale: an "enabling" pass can temporarily increase ``.text`` (e.g. ``licm``)
yet set up a strongly-linked following pass (``gvn``) that pays off. Plain beam
prunes such a path on immediate size; here the order-graph edge keeps it alive
so it can "lose now to win later".
"""
...
from llvm_ir.heuristics.translation_unit.segment_tree import segment_tree_beam_merge
...
HEURISTIC = "segment_tree_graph_beam"
...
def evaluate_for_benchmark(benchmark, function_results, bitcode_path, *, args):
    with tempfile.TemporaryDirectory(prefix="llvm-ir-segtreegb-") as tmp_str:
        ...
        for seg in selected:
            ...
            if best_passes:
                leaves.append((tuple(best_passes), best_size))
        leaf_evals = len(prefix_cache) - 1

        # ---- Phase 2: graph-informed include/skip beam merge ----
        leaves.sort(key=lambda item: (item[1], item[0]))
        if leaves:
            best_passes, best_size = segment_tree_beam_merge(
                leaves, measure, measure_exact,
                max_length=args.max_length, node_beam=max(2, node_beam_eff),
                edge_weight=edge_weight, graph_slots=args.node_graph_slots, graph_tol=args.node_graph_tol,
            )
        else:
        ...
        extra = {
            ...
            "node_beam": node_beam_eff, "node_graph_slots": args.node_graph_slots,
            "node_graph_tol": args.node_graph_tol,
...
def parse_args(argv: list[str] | None = None) -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Graph-informed beam segment-tree heuristic on whole TUs.")
    parser.add_argument("--comparison", required=True, type=Path)
    parser.add_argument("--algorithm", default="")
    parser.add_argument("--bitcode-dir", required=True, type=Path)
    parser.add_argument("--output", required=True, type=Path)
    parser.add_argument("--select-count", type=int, default=500)
    parser.add_argument("--gen-budget", type=int, default=4000)
    parser.add_argument("--segment-nodes", type=int, default=4)
    ...
    parser.add_argument("--node-beam", type=int, default=8, help="Base merge-node beam width (size-scaled).")
    parser.add_argument("--node-beam-min", type=int, default=2)
    parser.add_argument("--node-beam-size-ref", type=int, default=60000)
    parser.add_argument("--node-graph-slots", type=int, default=2,
                        help="Extra beam slots for graph-strong (but currently worse) paths.")
    parser.add_argument("--node-graph-tol", type=float, default=0.15,
                        help="Max fraction worse than best a graph-kept path may be (0=no cap).")
