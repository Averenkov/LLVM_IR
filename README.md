...
строится граф супер-вершин: одна супер-вершина равна короткой последовательности
pass-ов, а её вес равен измеренному `vertex_delta` на TU. Ребро между двумя
...
- `--superpath-min-segment-delta` отбрасывает сегменты без положительного
  measured delta (default `1`);
